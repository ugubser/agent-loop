  ...
  switch (entry.type) {
          ...
          if (debug) {
            ...
            for (const b of blocks) {
              if (b.type === "tool_result") {
                ...
                console.log(`${color}  [${b.tool_use_id}] ${c.slice(0, 500)}${C.reset}`);
              }
            }
          } else {
        ...
        if (debug) {
          ...
          for (const b of blocks) {
            if (b.type === "text" && b.text) {
              console.log(`${C.red}  ${String(b.text).slice(0, 1000)}${C.reset}`);
            } else if (b.type === "tool_use") {
              console.log(`${C.red}  CALL ${C.bold}${b.name}${C.reset}${C.red}(${JSON.stringify(b.input).slice(0, 500)})${C.reset}`);
            }
          }
        } else {
          for (const b of blocks) {
    ...
    case "tool_result": {
      ...
      if (debug && Array.isArray(blocks)) {
        ...
        console.log(`${prefix} ${C.green}tool_result: ${JSON.stringify(data).slice(0, 500)}${C.reset}`);
      } else {
